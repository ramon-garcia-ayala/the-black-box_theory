...
export async function scanSlides() {
  ...
  // Order by mega-group, then group-within-mega, then slide, then name as a tiebreaker.
  // A missing mega/group (legacy folders) sorts LAST, so a partial migration to the new
  // mNN_gNN_MM_* scheme keeps the structured slides up front and any not-yet-migrated
  // folders trailing.
  ...
  for (const f of folders) {
    const meta = f.meta;
    if (!meta.group && !meta.mega) { f.ord = 0; f.chapter = 0; f.colorKey = 0; f.shadeStep = 0; continue; }
  ...
  const slides = [];
  for (const { dirent: folder, meta, ord, chapter, colorKey, shadeStep } of folders) {
    ...
    slides.push({
      ...
      group: ord || 0,                                     // GLOBAL group ordinal (engine + scope), 0 = ungrouped
  ...
  if (looseItems.length) {
    ...
    slides.push({ index: lastIndex + 1, group: 0, gnum: 0, mega: 0, chapter: 0, colorKey: 0, shadeStep: 0, folder: '_loose', name: 'More', items: looseItems });
  }
